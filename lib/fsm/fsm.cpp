...
/**
 * @brief checks the patients movement and head position using the actigraph. if
 * the result is acceptable, the fsm moves to measure BP, otherwise it schedules
...
/**
 * @brief uses the PPG sensor to measure the paitent's blood pressure and moves
 * to the appropriate state based on the result
 */
void FSM::handleMeasureBP() {
    // /** @todo this should read the bp log and not just alert off of immediate
    //  * BP measurement */
