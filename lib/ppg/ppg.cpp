...
const byte LED_MODE =
    2; // this is red and IR. this was the best choice for us, but theoretically
       // IR gives good results for different skin tones.
...
float PPG::calculatePulsatilityIndex() {
    ...
    lastMinPeak = minPeak;
    lastMaxPeak = maxPeak;
    // PI = max peak - min peak / |mean|. this is passed into the regression
    // model to estimate SBP and DBP.
...
bloodPressure PPG::estimateBloodPressure(float pulsatilityIndex) const {
    ...
    results.systolic = SYSTOLIC_COEFF_M * pulsatilityIndex + SYSTOLIC_COEFF_B;
    results.diastolic =
