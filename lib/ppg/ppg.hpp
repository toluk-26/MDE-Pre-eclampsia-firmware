 ...
 * @description: Wrapper class for the MAX30101 sensor to make a BP estimation (using a pre-trained regression model).
 * @date 2026/05/04
 * @modified: 2026/05/04
 */
...
class PPG {
  public:
    ...
    /**
     * @brief runs one blocking PPG measurement. The sensor collects PPG data for one min
     * Extracts: min peak, max peak, mean value, and pulsatility index.
     * Estimates blood pressure using linear regression coefficients obtained from MATLAB.
  ...
  private:
     ...
     * this does not train a model on the microcontroller. it only uses fixed slope/intercept values found in matlab.
     */
